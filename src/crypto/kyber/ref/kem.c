...
bool crypto_kem_keypair(unsigned char *pk, unsigned char *sk) {
    size_t i;
    if (!indcpa_keypair(pk, sk))
        return false;
    for (i = 0; i < KYBER_INDCPA_PUBLICKEYBYTES; i++)
        sk[i + KYBER_INDCPA_SECRETKEYBYTES] = pk[i];
    hash_h(sk + KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
    /* Value z for pseudo-random output on reject */
    get_random_bytes(sk + KYBER_SECRETKEYBYTES - KYBER_SYMBYTES, KYBER_SYMBYTES);
    return true;
}
...
bool crypto_kem_dec(unsigned char *ss,
    ...
    /* coins are in kr+KYBER_SYMBYTES */
    if (!indcpa_enc(cmp, buf, pk, kr + KYBER_SYMBYTES))
        return false;
